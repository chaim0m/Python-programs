...
def correctGuess(secretWord, guess):
    '''
    secretWord: string, the word the user is guessing
    guess: the letter guessed on a given round by a user
    returns:string indicating whether the letter guessed 
    was a correct guess or not
    '''
...
while c>0:
    ...
    guess = guess.lower()
    a = correctGuess(secretWord, guess)
    if a is False:
        if guess in guessed:
        ...
        if guess in guessed:
            ...
            print('-----------')
    if isWordGuessed(secretWord, guessed) is True:
